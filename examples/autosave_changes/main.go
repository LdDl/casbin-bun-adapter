...
func main() {
	...
	*/
	...
	found, err = enforcer.Enforce("alice", "data2", "write") // Should be TRUE
	if err != nil {
		log.Println("Error on enforcing", err)
		return
	}
	...
	/* Enable autosave. Start listening changes to the enforcer and update database tables */
	// Be careful when making application logic: make sure that you not going to use `StartUpdatesListening` in casbin along with listening to database updates since it could cause infinity recursion
	enforcer.EnableAutoSave(true)

	/* Check add policy update */
	time.Sleep(100 * time.Millisecond)
	_, err = enforcer.AddPolicy("alice", "data2", "write", "deny")
	if err != nil {
		log.Println("Error on adding policy to the enforcer", err)
		return
	}

	time.Sleep(100 * time.Millisecond)
	found, err = enforcer.Enforce("alice", "data2", "write") // Should be FALSE due new deny rule above
	if err != nil {
		log.Println("Error on enforcing", err)
		return
	}
